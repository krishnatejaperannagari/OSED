...
def _call_wp_api(action):
    api_url = pylons.config.get('ckanext.osed.wp_ajax_url', None)
    try:
        """
...
def get_dataset_terms_of_use(pkg):
    rights = logic.get_action('osed_dataset_terms_of_use')({}, {'id': pkg})
    return rights['dataset_rights']


def get_dataset_by_identifier(identifier):
    try:
        return logic.get_action('osed_dataset_by_identifier')(
            {'for_view': True},
