...
from ckanext.osed import validators as v
from ckanext.osed.logic import (
    osed_dataset_count, osed_dataset_terms_of_use,
    osed_dataset_by_identifier, osed_content_headers
)
from ckanext.osed.helpers import (
...
class OsedPlugin(plugins.SingletonPlugin):
    ...
    def update_config(self, config_):
        ...
        toolkit.add_resource('fanstatic', 'osed')
    ...
    def get_validators(self):
        return {
            ...
            'osed_multiple_choice': v.osed_multiple_choice,
            'osed_unique_identifier': v.osed_unique_identifier,
    ...
    def dataset_facets(self, facets_dict, package_type):
        ...
        facets_dict['license_id'] = plugins.toolkit._('Terms of use')
    ...
    def get_actions(self):
        ...
        """
        return {
            'osed_dataset_count': osed_dataset_count,
            'osed_dataset_terms_of_use': osed_dataset_terms_of_use,
            'osed_dataset_by_identifier': osed_dataset_by_identifier,
            'osed_content_headers': osed_content_headers,
        }
...
class OsedLanguagePlugin(plugins.SingletonPlugin):
...
class OsedGroupPlugin(OsedLanguagePlugin):
    plugins.implements(plugins.IGroupController, inherit=True)

    # IGroupController
    def before_view(self, pkg_dict):
        return super(OsedGroupPlugin, self).before_view(pkg_dict)


class OsedOrganizationPlugin(OsedLanguagePlugin):
    plugins.implements(plugins.IOrganizationController, inherit=True)

    # IOrganizationController

    def before_view(self, pkg_dict):
        return super(OsedOrganizationPlugin, self).before_view(pkg_dict)


class OsedResourcePlugin(OsedLanguagePlugin):
    plugins.implements(plugins.IResourceController, inherit=True)

    # IResourceController

    def before_show(self, pkg_dict):
        return super(OsedResourcePlugin, self).before_view(pkg_dict)

    def _ignore_field(self, key):
        return key == 'tracking_summary'


class OsedPackagePlugin(OsedLanguagePlugin):
